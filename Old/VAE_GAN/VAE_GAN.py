import sys
home_env = '../'
sys.path.append(home_env)

from Resources import Networks as myNN
from Resources import Datasets as myDS

import os
import numpy as np

import torch as T
import torch.nn as nn
import torch.optim as optim
import torch.nn.functional as F
from torch.utils.data import Dataset, DataLoader

from tqdm import tqdm
from collections import OrderedDict, deque

class Agent(object):
    def __init__(self,
                 name,
                 net_dir,
                 \
                 dataset_name,
                 data_dims, flatten_data,
                 latent_dims,
                 condit_dims, targ_onehot,
                 clamp_out,
                 \
                 burn_in, swap_time,
                 \
                 V_lr,
                 V_act,
                 V_mlp_layers, V_drpt, V_lnrm,
                 V_cnn_layers,
                 \
                 D_lr,
                 D_act,
                 D_mlp_layers, D_drpt, D_lnrm,
                 D_cnn_layers,
                 \
                 reg_weight, gan_weight,
                 \
                 batch_size, n_workers,
                 ):

        ## Setting all class variables
        self.__dict__.update(locals())

        ## We check if we need a convolutional neural network
        do_cnn = not( len(data_dims)==flatten_data==1 )
        self.train_gen = True

        ## Getting the training and testing dataloaders
        self.train_loader, self.test_loader = myDS.load_dataset( dataset_name, batch_size, n_workers )

        ## The VAE network
        self.VAE = myNN.VAE_Network( name+"_VAE", net_dir, do_cnn,
                                     data_dims, latent_dims, condit_dims, clamp_out,
                                     V_act, V_mlp_layers, V_drpt, V_lnrm,
                                     V_cnn_layers )

        ## The Discriminator network
        self.DIS = myNN.DIS_Network( name+"_DIS", net_dir, do_cnn,
                                     data_dims, condit_dims,
                                     D_act, D_mlp_layers, D_drpt, D_lnrm,
                                     D_cnn_layers )

        ## The optimiser and reconstruction loss function
        self.VAE_optimiser = optim.Adam( self.VAE.parameters(), lr=V_lr )
        self.DIS_optimiser = optim.Adam( self.DIS.parameters(), lr=D_lr )
        self.rec_loss_fn = nn.BCELoss( reduction="sum" )
        self.gan_loss_fn = nn.CrossEntropyLoss( reduction="sum" )

        ## Running history of the test and training losses per epoch, for graphing
        self.epochs_trained = 0
        self.trn_vls_hist = deque( maxlen=100 )
        self.tst_vls_hist = deque( maxlen=100 )
        self.trn_acc_hist = deque( maxlen=100 )
        self.tst_acc_hist = deque( maxlen=100 )

        ## A list of examples to save for use in visualisation of the reconstruction and the latent space
        idxes = np.random.randint(0,1000,4)
        self.recon_data     = T.stack([self.test_loader.dataset[i][0] for i in idxes ])
        self.recon_targets  = T.tensor([self.test_loader.dataset[i][1] for i in idxes ], dtype=T.int64)
        self.latent_means   = 0
        self.latent_targets = 0

    def save_models(self, flag=""):
        self.VAE.save_checkpoint(flag)

    def load_models(self, flag=""):
        self.VAE.load_checkpoint(flag)

    def prepare_conditional(self, targets):
        """ This is used to modify the targets into conditional information,
            particularly for one-hot encoding categorial varaibles (MNIST, CIFAR)
            This function is called even if the no conditional information is used.
        """
        if self.condit_dims == 0:
            return None

        condit = targets.to(self.VAE.device)
        if self.targ_onehot:
            condit = F.one_hot(condit, num_classes = self.condit_dims)
        return condit

    def prepare_data(self, data):
        """ This is used to modify the input data for the network,
            particularly for flattening images when using an MLP
        """
        data = data.to(self.VAE.device)
        if self.flatten_data:
            data = data.view( data.size(0), -1 )
        return data

    def train(self):
        """ This function performs one epoch of training on data provided by the train_loader
        """
        ## Put both networks into training mode
        self.VAE.train()
        self.DIS.train()

        ## We work out if we are to train the discriminator or the generator based on how well the discriminator is doing
        if self.epochs_trained<self.burn_in:
            self.train_gen = True
        else:
            if self.trn_acc_hist[-1]>0.80 and self.train_gen==False:
                self.train_gen = True
            elif self.trn_acc_hist[-1]<0.60 and self.train_gen==True:
                self.train_gen = False

        run_vae_loss = 0
        run_dis_acc  = 0
        print( "Training Generator" if self.train_gen else "Training Discriminator" )
        for (data, targets) in tqdm(self.train_loader, desc="Training", ncols=80, unit=""):
            this_b_size = len(data)

            ## We prepare the input and conditional data (flatten, one-hot, move to device, etc)
            data   = self.prepare_data(data)
            condit = self.prepare_conditional(targets)

            ## We calculate the reconstructed output and latent stats of the VAE
            reconstructions, means, log_stds = self.VAE(data, condit)

            ## We calculate the total loss of the batch by combining the reconstruction error and the regulariation
            rec_loss = self.rec_loss_fn(reconstructions, data)
            kld_loss = - 0.5 * T.sum( 1 + 2*log_stds - means*means - (2*log_stds).exp() )
            vae_loss = rec_loss + self.reg_weight * kld_loss
            run_vae_loss += vae_loss.item()

            ## After a burn in period we turn on the discriminator
            if self.epochs_trained>=self.burn_in:
                ## The inputs and the outputs are pushed together then randomly split apart
                combined    = T.cat( (data.unsqueeze(0), reconstructions.unsqueeze(0) ), 0 )
                identities  = T.LongTensor(this_b_size).random_(0, 2).to(self.DIS.device)
                batch_idxes = list(range(this_b_size))
                input1 = combined[identities, batch_idxes]
                input2 = combined[1-identities, batch_idxes]

                dis_output = self.DIS( input1, input2, condit )
                gan_loss   = self.gan_loss_fn(dis_output,identities)

                ## We see how many the discriminator got correct
                _, predicted = T.max(dis_output.data, 1)
                run_dis_acc += (predicted == identities).sum().item()

                ## And we modify the loss function
                vae_loss -= self.gan_weight*gan_loss

            ## We perform gradient descent on the discriminator
            if self.train_gen:
                self.VAE_optimiser.zero_grad()
                vae_loss.backward()
                self.VAE_optimiser.step()

            ## Perform gradient descent on the discriminator generator
            else:
                self.DIS_optimiser.zero_grad()
                gan_loss.backward()
                self.DIS_optimiser.step()

        ## Update the epoch deque and the epoch counter
        self.trn_vls_hist.append( run_vae_loss / len(self.train_loader.dataset) )
        self.trn_acc_hist.append( run_dis_acc  / len(self.train_loader.dataset) )
        self.epochs_trained += 1

    def test(self):
        """ This function performs one epoch of testing on data provided by the test_loader.
            It is basically the same as the train functin above but without the gradient desc
        """
        self.VAE.eval()
        with T.no_grad():
            run_vae_loss = 0
            run_dis_acc  = 0
            for (data, targets) in tqdm(self.test_loader, desc="Testing", ncols=80, unit=""):
                this_b_size = len(data)

                data   = self.prepare_data(data)
                condit = self.prepare_conditional(targets)

                reconstructions, means, log_stds = self.VAE(data, condit)

                rec_loss = self.rec_loss_fn(reconstructions, data)
                kld_loss = - 0.5 * T.sum( 1 + 2*log_stds - means*means - (2*log_stds).exp() )
                vae_loss = rec_loss + self.reg_weight * kld_loss
                run_vae_loss += vae_loss.item()

                if self.epochs_trained > self.burn_in:
                    combined    = T.cat( (data.unsqueeze(0), reconstructions.unsqueeze(0) ), 0 )
                    identities  = T.LongTensor(this_b_size).random_(0, 2).to(self.DIS.device)
                    batch_idxes = list(range(this_b_size))
                    input1 = combined[identities, batch_idxes]
                    input2 = combined[1-identities, batch_idxes]

                    dis_output = self.DIS( input1, input2, condit )
                    gan_loss   = self.gan_loss_fn(dis_output,identities)

                    _, predicted = T.max(dis_output.data, 1)
                    run_dis_acc += (predicted == identities).sum().item()

            self.tst_vls_hist.append( run_vae_loss / len(self.test_loader.dataset) )
            self.tst_acc_hist.append( run_dis_acc  / len(self.test_loader.dataset) )

        ## Graphs and visualisations are based on the final batch in the test set
        self.latent_means = means.cpu()
        self.latent_targets = targets

    def visualise_recreation(self):
        """ This function returns reconstructions of 4 selected examples and is called
            during training after each epoch. Good for images.
        """
        with T.no_grad():
            data = self.prepare_data( self.recon_data )
            condit_info = self.prepare_conditional(self.recon_targets)
            reconstructions, *_ = self.VAE(data, condit_info)
            return reconstructions.view(self.recon_data.shape).cpu()
